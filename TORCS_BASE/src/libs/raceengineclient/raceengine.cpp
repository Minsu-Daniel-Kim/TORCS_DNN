...
/* Connect to a socket for IPC */
zmq::context_t context(1);
zmq::socket_t ipc_socket (context, ZMQ_DEALER);
char identity[10] = {};
...
ReStart(void)
{
    ...
    ipc_socket.setsockopt(ZMQ_IDENTITY, identity, strlen(identity));
		ipc_socket.connect("ipc:///tmp/feeds/0");

		// Create CSV file and label columns.
		tRmMovieCapture	*capture = &(ReInfo->movieCapture);
		char buf[100];
		int bufsize = 100;
		snprintf(buf, bufsize, "%s/capture_info.csv", capture->outputBase);
		ofstream myfile;
		myfile.open(buf, fstream::app);
		myfile << "index, steer, gear, accel, brake, clutch\n";
		myfile.close();
}
...
static void
reTransmitVisionData(void)
{
	/**
	TODO:
	When "Ready set Go" is ended, call reMovieCapture()
	**/

	/** Capture Image & GrayScale/Resize **/
	int sw, sh, vw, vh;
	tRmMovieCapture	*capture = &(ReInfo->movieCapture);
	tSituation *s = ReInfo->s;

	// tCarElt Pointer that points to our driving car
	tCarElt *car = s->cars[0];

	GfScrGetSize(&sw, &sh, &vw, &vh);
	
	size_t size = vw * vh * 3;
	size_t dst_size = 280 * 210 * 3;
	unsigned char * img;
	unsigned char * dst_img;
	img = (unsigned char*) malloc(size);
	dst_img = (unsigned char*) malloc(dst_size);
  if (img == NULL || dst_img == NULL) { return; }
	glPixelStorei(GL_PACK_ROW_LENGTH, 0);
	glPixelStorei(GL_PACK_ALIGNMENT, 1);
	glReadBuffer(GL_BACK);
	glReadPixels((sw-vw)/2, (sh-vh)/2, vw, vh, GL_RGB, GL_UNSIGNED_BYTE, (GLvoid*)img);
	
	/** Grayscale, Resize, Flip Image with OpenCV **/	
	const int BUFSIZE = 100;
	char capture_filename[BUFSIZE];
	snprintf(capture_filename, BUFSIZE, "/home/yeonhoo/captures/capture_%d.png", counter);
 	ReImagePreprocess(img, dst_img, vw, vh, counter);

 	/** Parse Data into CSV **/
 	char csv_filename[BUFSIZE];
	snprintf(csv_filename, BUFSIZE, "%s/capture_info.csv", capture->outputBase);
	ofstream myfile;
	myfile.open(csv_filename, fstream::app);
	myfile << capture_filename << "," << car->_steerCmd <<","<< car->_gearCmd <<
	"," << car->_accelCmd << "," << car->_brakeCmd << "," <<
	car->_clutchCmd << std::endl;
	myfile.close();

	/** IPC Data Transmission **/
	// zmq::pollitem_t items[] = {ipc_socket, 0, ZMQ_POLLIN, 0};
 //  try {
 //    for (int i = 0; i < 10; ++i) {
 //      // Wait for Response for 10 milliseconds
 //      zmq::poll(items, 1, 10);
 //      if (items[0].revents & ZMQ_POLLIN) {
 //      	// Get 5 parameters and set car's parameter at here.
 //        // printf("\n%s ", identity);
 //        // s_dump(ipc_socket);
 //      }
 //    }
 //    std::cout << "Sending Request…" << std::endl;
 //    // compressBZ2("~/captures/capture.txt");
 //    ipc_socket.send(dst_img, size);
 //  }
 ...
 //  }
  ...
  // TODO: Should compress the data with Zlib to boost network I/O
  // When response from server is received, change car's ctrl
	counter = counter+1;
	free(img);
	free(dst_img);
}
